...
def print_results(results_dic, results_stats_dic, model, 
    ...
    """   
    print(f"architecture used :: {model.upper()}")
    print("="*10,'printing stats',"="*10)
    ...
    for i in results_stats_dic.keys():
        if 'pct' in i: 
            print(f"{results_stats_dic[i]} % {i}")   
    if(print_incorrect_dogs):
        print("="*10,'printing incorrect dogs',"="*10)
        if results_stats_dic['n_images'] == results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs']:
            print("All dogs correctly classified")
        else:
            for key,val in results_dic.items():
                if val[3] != val[4]:
                    print(f"image {key.upper()} with label {val[0].upper()} predicted as {val[1].upper()}")
    if(print_incorrect_breed):
        print("="*10,'printing incorrect breeds',"="*10)
        if results_stats_dic['n_correct_dogs'] == results_stats_dic['n_correct_breed']:
            print("All breeds correctly classified")
        else:
            for key,val in results_dic.items():
